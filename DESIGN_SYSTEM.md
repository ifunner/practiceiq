...
The GolfIQ design language is built for golf tools that feel like premium course-side instruments: dark felt greens, warm ivory type, and a single flag-yellow accent for the moments that matter (aim point, cup, primary action). GreenIQ is the reference implementation of this system.
...
All tokens live on `:root` in `styles.css`.
...
Body uses a radial gradient over felt:

```css
background:
  radial-gradient(120% 60% at 50% -10%, #16382d 0%, var(--felt) 42%, var(--felt-deep) 100%);
```

Special card variant `.read` uses a darker inset gradient:
...
Space Grotesk applies to: `.brand h1`, `.card h2`, `summary`, `.tabbar button`.

Monospace applies to: `.mono`, stepper inputs, `.readout-slope`, `.bigval`, chips’ numeric values, insight stats.
...
- Utility cluster right: unit toggle (`.ghost` small button) + `.pill` badge
...
Auto-dismiss ~1.6s. Position: fixed, centered above tab bar (`bottom: 24px`).
...
| `app.js` | SVG chart generation, dynamic states |
...
1. Copy `:root` tokens from `styles.css`.
2. Load Space Grotesk (500, 700) from Google Fonts.
3. Use `--felt-deep` page background with felt radial gradient.
4. Stack content in `.card` sections inside a 520px column.
5. Put primary actions in `.primary` buttons; keep yellow scarce.
6. Render measurements in mono; labels in uppercase sage eyebrows.
7. Anchor navigation in a blurred `.tabbar` with safe-area padding.
